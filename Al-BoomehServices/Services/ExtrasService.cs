...
namespace Al_BoomehDAL.Classes
{
    public class ExtrasService
    {
        ...
        public ExtrasService(AppDbContext context,ILogger<ExtrasService> logger)
        {
            _logger = logger;
            _context = context;
        }
        ...
        public async Task<bool> UpdateExtra(int extraId, UpdateExtraDTO extraDTO)
        {
            ...
            if (extra == null)
            {
                _logger.LogWarning("failed to update extra info for id {ExtraID}, no data found for update",
                   extraId);
                throw new BusinessRuleException($"failed to update extra info , no data found for update");
            }
