...
namespace Al_BoomehDAL.Classes
{
    ...
    public class ProductsService
    {
        ...
        {
            if (productDTO == null) throw new BusinessRuleException($"failed to add product to store , no data found for update");
            if (await IsProductExistByNameInStore(productDTO.StoreId, productDTO.ProductName)) throw new BusinessRuleException($"failed to add product to store ,product already exist");
        ...
        public async Task<bool> UpdateProduct(int productId, UpdateProductDTO productDTO)
        {
            ...
            if (product == null)
            {
                _logger.LogWarning("failed to update product info for id {ProductID}, no data found for update",
                   productId);
                throw new BusinessRuleException($"failed to update product info , no data found for update");
            }
