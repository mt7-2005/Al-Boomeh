...
namespace Al_BoomehDAL.Classes
{
    ...
    _context.Database.BeginTransactionAsync();
            try
            {
                ...
                var currentStatus = (enStatus)order.Status;
                var targetStatus = statusdto.Status;

                bool isAdmin = _currentUser.Role == Al_BoomehDAL.Models.User.UserRole.Admin;


                if (!isAdmin)
                {
                    var allowedTargets = AllowedTransitions.TryGetValue(currentStatus, out var targets)
                        ? targets
                        : Array.Empty<enStatus>();

                    if (!allowedTargets.Contains(targetStatus))
                    {
                        throw new BusinessRuleException(
                            $"Cannot move an order from {currentStatus} to {targetStatus}. " +
            ...
            {
                await transaction.RollbackAsync();
                _logger.LogWarning(ex, "Failed to update status for order {OrderId}", orderId);
                throw new BusinessRuleException("Failed to update order status.");
            }
   ...
   _context.Database.BeginTransactionAsync();
            try
            {
                ...
                if (product == null)
                {
                    _logger.LogWarning("AddLineToOrder failed: product {ProductId} not found ",
                       lineDTO.ProductId);
                    throw new NotFoundException($"AddLineToOrder failed: product not found");

                }
                if (product.StockQuantity < (int)lineDTO.Quantity || product.IsOutOfStock)
                {
                    errors["Product"] = ["AddLineToOrder failed: product Out Of Stock"];
        ...
        public async Task<int> AddLineToCart(int orderId,CreateOrderLineDTO lineDTO)
        {
            ...
            if (product == null)
            {
                ...
                throw new NotFoundException("Failed to add line , product not found");
            }
            var line = new OrderLine
            {
                OrderId = orderId,
